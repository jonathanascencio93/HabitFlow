...
import { StyleSheet, View, Text, TextInput, TouchableOpacity, SafeAreaView, KeyboardAvoidingView, Platform, ScrollView, Switch, Alert } from 'react-native';
    ...
    useEffect(() => {
        if (id) {
            ...
            if (habit) {
                ...
                setPoints(habit.pointsValue.toString());
                if (habit.recurrence) {
                    setRecurrenceType(habit.recurrence.type);
                    if (habit.recurrence.type === 'specific_days') setSelectedDays(habit.recurrence.daysOfWeek || [1, 2, 3, 4, 5]);
                    if (habit.recurrence.type === 'weekly' && habit.recurrence.daysOfWeek?.[0] !== undefined) setWeeklyDay(habit.recurrence.daysOfWeek[0]);
                    if (habit.recurrence.type === 'monthly' && habit.recurrence.dayOfMonth) setDayOfMonth(habit.recurrence.dayOfMonth.toString());
                } else {
